...
export function formatEvent(state: GameState, e: GameEvent): string | null {
  switch (e.type) {
    ...
    case 'ShotResolved':
      return `  ${name(state, e.attackerId)} (${score(e.attackScore, e.attackBonus, [['long range', e.rangePenalty], ['cover', e.coverPenalty]], [['big target', e.bigTarget], ['flying target', e.flyingTarget], ['opportunist', e.attackOpportunist]])}) ${e.aimPenalty ? 'takes an aimed shot at' : 'shoots'} ${name(state, e.targetId)} (${score(e.defenseScore, e.defenseBonus, [['aimed at', e.aimPenalty]])}) → ${e.result}${gore(e)}`;
