...
export function formatEvent(state: GameState, e: GameEvent): string | null {
  switch (e.type) {
    ...
    case 'AttackResolved':
      return `  ${name(state, e.attackerId)} (${score(e.attackScore, e.attackBonus, [['outnumbered', e.attackOutnumbered]], [['size', e.attackBig], ['flying', e.attackFly]])}) ${e.powerPenalty ? 'lands a power blow on' : 'attacks'} ${name(state, e.targetId)} (${score(e.defenseScore, e.defenseBonus, [['outnumbered', e.defenseOutnumbered], ['power blow', e.powerPenalty]], [['size', e.defenseBig]])}) → ${e.result}${gore(e)}`;
    case 'ShotResolved':
    ...
    case 'FreeHackResolved':
      return `  ${name(state, e.attackerId)} (${score(e.attackScore, e.attackBonus, [['outnumbered', e.attackOutnumbered]], [['size', e.attackBig], ['flying', e.attackFly]])}) takes a free hack at ${name(state, e.targetId)} (${score(e.defenseScore, e.defenseBonus, [['outnumbered', e.defenseOutnumbered]], [['size', e.defenseBig]])}) → ${e.result === 'defenderRecoiled' ? 'slips away' : e.result}${gore(e)}`;
    case 'GuardDeclared':
      return `  ${name(state, e.unitId)} raises guard`;
    case 'GuardRiposte':
      return `  ${name(state, e.guardId)} (${score(e.guardScore, e.guardBonus, [['outnumbered', e.guardOutnumbered]], [['size', e.guardBig], ['flying', e.guardFly]])}) ripostes ${name(state, e.attackerId)} (${score(e.attackerScore, e.attackerBonus, [['outnumbered', e.attackerOutnumbered]], [['size', e.attackerBig]])}) → ${e.result === 'clash' ? 'attack goes through' : e.result}${gore(e)}${e.prevented ? ' (attack stopped)' : ''}`;
    case 'ToughnessSaved':
