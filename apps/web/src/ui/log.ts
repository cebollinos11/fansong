...
export function formatEvent(state: GameState, e: GameEvent): string | null {
  switch (e.type) {
    ...
    case 'AttackResolved':
      return `  ${name(state, e.attackerId)} (${score(e.attackScore, e.attackBonus, [['outnumbered', e.attackOutnumbered]], [['size', e.attackBig], ['flying', e.attackFly], ['mounted', e.attackMounted]])}) ${e.powerPenalty ? 'lands a power blow on' : 'attacks'} ${name(state, e.targetId)} (${score(e.defenseScore, e.defenseBonus, [['outnumbered', e.defenseOutnumbered], ['power blow', e.powerPenalty]], [['size', e.defenseBig], ['mounted', e.defenseMounted]])}) → ${e.result}${gore(e)}`;
    case 'ShotResolved':
      return `  ${name(state, e.attackerId)} (${score(e.attackScore, e.attackBonus, [['long range', e.rangePenalty], ['cover', e.coverPenalty]], [['big target', e.bigTarget], ['flying target', e.flyingTarget]])}) ${e.aimPenalty ? 'takes an aimed shot at' : 'shoots'} ${name(state, e.targetId)} (${score(e.defenseScore, e.defenseBonus, [['aimed at', e.aimPenalty]])}) → ${e.result}${gore(e)}`;
    case 'FreeHackResolved':
      return `  ${name(state, e.attackerId)} (${score(e.attackScore, e.attackBonus, [['outnumbered', e.attackOutnumbered]], [['size', e.attackBig], ['flying', e.attackFly], ['mounted', e.attackMounted]])}) takes a free hack at ${name(state, e.targetId)} (${score(e.defenseScore, e.defenseBonus, [['outnumbered', e.defenseOutnumbered]], [['size', e.defenseBig], ['mounted', e.defenseMounted]])}) → ${e.result === 'defenderRecoiled' ? 'slips away' : e.result}${gore(e)}`;
    case 'GuardDeclared':
      return `  ${name(state, e.unitId)} raises guard`;
    case 'GuardRiposte':
      return `  ${name(state, e.guardId)} (${score(e.guardScore, e.guardBonus, [['outnumbered', e.guardOutnumbered]], [['size', e.guardBig], ['flying', e.guardFly], ['mounted', e.guardMounted]])}) ripostes ${name(state, e.attackerId)} (${score(e.attackerScore, e.attackerBonus, [['outnumbered', e.attackerOutnumbered]], [['size', e.attackerBig], ['mounted', e.attackerMounted]])}) → ${e.result === 'clash' ? 'attack goes through' : e.result}${gore(e)}${e.prevented ? ' (attack stopped)' : ''}`;
    case 'ToughnessSaved':
