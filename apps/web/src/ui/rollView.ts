...
export function describeCombat(e: Combat, after: readonly GameEvent[] = []): OpposedRoll {
  ...
  if (e.type === 'GuardRiposte') {
    ...
    a = side(e.guardId, 'Riposte', e.guardDie, e.guardScore, [['High ground', e.guardBonus], ['Size', e.guardBig], ['Swoop', e.guardFly], ['Mounted', e.guardMounted], ['Outnumbered', minus(e.guardOutnumbered)]], oa);
    b = side(e.attackerId, 'Attack', e.attackerDie, e.attackerScore, [['High ground', e.attackerBonus], ['Size', e.attackerBig], ['Mounted', e.attackerMounted], ['Outnumbered', minus(e.attackerOutnumbered)]], ob);
    ...
    const [oa, ob] = outcomes(e.attackScore, e.defenseScore);
    a = side(e.attackerId, hack ? 'Free hack' : power ? 'Power blow' : 'Attack', e.attackDie, e.attackScore, [['High ground', e.attackBonus], ['Size', e.attackBig], ['Swoop', e.attackFly], ['Mounted', e.attackMounted], ['Outnumbered', minus(e.attackOutnumbered)]], oa);
    b = side(e.targetId, hack ? 'Leaving' : 'Defend', e.defenseDie, e.defenseScore, [['High ground', e.defenseBonus], ['Size', e.defenseBig], ['Mounted', e.defenseMounted], ['Outnumbered', minus(e.defenseOutnumbered)], ['Power blow', minus(power)]], ob);
  }
