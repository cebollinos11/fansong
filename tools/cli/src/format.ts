...
export function formatEvent(state: GameState, e: GameEvent): string {
  switch (e.type) {
    ...
    case 'AttackResolved':
      return `  ${name(state, e.attackerId)} ${e.powerPenalty ? 'lands a power blow on' : 'attacks'} ${name(state, e.targetId)}: ${e.attackScore} vs ${e.defenseScore} (d${e.attackDie}/d${e.defenseDie})${mods([['outnumbered', e.attackOutnumbered && -e.attackOutnumbered], ['size', e.attackBig], ['swoop', e.attackFly], ['mounted', e.attackMounted], ['opportunist', e.attackOpportunist], ['foe outnumbered', e.defenseOutnumbered && -e.defenseOutnumbered], ['foe size', e.defenseBig], ['foe mounted', e.defenseMounted], ['foe opportunist', e.defenseOpportunist], ['foe power-blown', e.powerPenalty && -e.powerPenalty]])} -> ${e.result}${gore(e)}`;
    case 'ShotResolved':
      return `  ${name(state, e.attackerId)} ${e.aimPenalty ? 'takes an aimed shot at' : 'shoots'} ${name(state, e.targetId)}: ${e.attackScore} vs ${e.defenseScore} (d${e.attackDie}/d${e.defenseDie})${mods([['big target', e.bigTarget], ['flying target', e.flyingTarget], ['opportunist', e.attackOpportunist], ['long range', e.rangePenalty && -e.rangePenalty], ['cover', e.coverPenalty && -e.coverPenalty], ['foe aimed at', e.aimPenalty && -e.aimPenalty]])} -> ${e.result}${gore(e)}`;
