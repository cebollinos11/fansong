...
export function validateMap(
  ...
  if (!deployZonesConnected(map)) errors.push('deploy hexes are not all connected by passable ground');

  // --- Objectives -----------------------------------------------------------
  const { flags, hill, conquest } = map.objectives;

  if (flags) {
    flags.forEach((v, p) => checkZone(`player ${p} flag base`, [v]));
...
/**
 * Is every deploy hex in one walkable region? A deploy hex walled off from the
 * rest would strand a unit (and could leave annihilation unwinnable). Only
 * called once the zones themselves are known to be passable or reported.
 */
function deployZonesConnected(map: MapDef): boolean {
  ...
  const hexes = [...map.deployZones[0], ...map.deployZones[1]].filter(
    (v) => board.inBounds(v) && !board.isBlocked(v),
  );
  const first = hexes[0];
  if (!first) return true; // nothing placeable — the zone-size errors already say so
  const reach = board.reachableWithin(first, map.width * map.height);
  reach.add(vecKey(first));
  return hexes.every((v) => reach.has(vecKey(v)));
}
